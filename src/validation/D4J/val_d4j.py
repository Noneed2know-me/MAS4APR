...
def checkout_defects4j_project(current_bug, project_dir):
    ...
    command = "/MAS4APR/defects4j/framework/bin/defects4j checkout " + " -p " + project + " -v " + bug_id + 'b'  + " -w " + project_dir
...
def defects4j_test_suite(project_dir, timeout=1000):
    ...
    out, err = command_with_timeout(["/MAS4APR/defects4j/framework/bin/defects4j", "test", "-r"], timeout)
...
def defects4j_export_trigger(project_dir, timeout=90):
    os.chdir(project_dir)
    out, err = command_with_timeout(["/MAS4APR/defects4j/framework/bin/defects4j", "export", "-p", "tests.trigger"], timeout)
    return out, err


def defects4j_export_relevant(project_dir, timeout=90):
    os.chdir(project_dir)
    out, err = command_with_timeout(["/MAS4APR/defects4j/framework/bin/defects4j", "export", "-p", "tests.relevant"], timeout)
    return out, err


def defects4j_test_one(project_dir, test_case, timeout=100):
    os.chdir(project_dir)
    out, err = command_with_timeout(["/MAS4APR/defects4j/framework/bin/defects4j", "test", "-t", test_case], timeout)
    return out, err
...
def validate_patches_per_bug(candidate_patch):
    ...
    for curr_patch_code in val_info.patches:
        ...
        # 新增: 如果补丁状态为 PLAUSIBLE，添加到 plausible_patches 列表
        if curr_patch_summary['patch_status'] == 'PLAUSIBLE':
            print("Plausible found")
            plausible_patches.append({
                'patch_code': curr_patch_summary['patch_code'],
                'bug_name': curr_patch_summary['bug_name']
            })
    ...
    val_time.print_validation_time_info(val_info.curr_bug)
    val_info.save_validation_results(done=True)
    plausible_path = "./plausible"

    # 新增: 保存 PLAUSIBLE 补丁到文件
    if plausible_patches:
        
        plausible_file_path = f"/MAS4APR/src/validation/D4J/plausible/{val_info.curr_bug}-plausible.json"
        print(plausible_file_path)
